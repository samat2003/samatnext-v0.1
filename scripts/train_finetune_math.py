...
class ChatDataset(torch.utils.data.Dataset):
    def __init__(self, tokenizer, rank, world_size, seq_len=2048):
        ...
        # Load MetaMathQA - 395k math reasoning problems
        if is_main_process_hack():
            print("Loading meta-math/MetaMathQA (sampling 20k for speed)...")
        ds_math = load_dataset("meta-math/MetaMathQA", split="train")
        
        import random
        random.seed(42)
        indices = random.sample(range(len(ds_math)), 20000)  # Sample 20k for ~1 hour run
        ...
        if is_main_process_hack():
            print(f"  TOTAL: {len(self.examples)} math training examples loaded!")
...
def train():
    ...
    CKPT_DIR = os.path.join(ROOT_DIR, "checkpoints", "finetune_math")
    # Load from the CodeFeedback fine-tuned weights — this is Stage 2 of continual learning
    PRETRAIN_CKPT = os.path.join(ROOT_DIR, "checkpoints", "finetune", "step_1500_final.pt")
    
    if is_main_process:
        os.makedirs(CKPT_DIR, exist_ok=True)
        print(f"Starting Math Fine-Tuning (Stage 2 - Continual Learning) with {world_size} GPUs.")
    ...
    # Standard LLM Warmup
    total_steps = 625  # ~1 Epoch over 20k math examples with grad_accum=32
