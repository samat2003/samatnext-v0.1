...
- **License:** Apache License 2.0 (under Qwen2.5-Coder model terms).
- **Redistribution:** Allowed. Committed under `data/stage5_teacher_distill.jsonl` for exact baseline comparison.
- **Upstream License & Output Rights:** The upstream Qwen2.5-Coder-3B-Instruct model is released under the **Apache License 2.0**. The outputs generated by this model are permitted for research and commercial use. Data generated under this model is distributed under the Apache-2.0 terms, provided that usage complies with upstream copyright notices and standard model usage restrictions (avoiding use for illegal activity, malware generation, etc.).
