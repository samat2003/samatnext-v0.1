...
class MixedStageDataset(Dataset):
    def __init__(self, tok, max_seq):
        ...
        print(f"Loaded: Stage 6A ({len(s6)-skipped_6}), Stage 3 ({len(s3)-skipped_3}), Stage 5 ({len(s5)-skipped_5})")
        
        # Now create the mixed dataset (80/10/10) dynamically when sampling
        self.s6_data = [e for e in self.examples if e.get("source", "stage6") == "stage6"]
...
def main():
    ...
    while opt_step < TOTAL_STEPS:
        ...
        if opt_step % 100 == 0:
            ...
            stage6_pass = metrics.get("Stage 6A Natural Holdout", {}).get("pass_rate", 0)
            he5_pass = metrics.get("HumanEval 5", {}).get("pass_rate", 0)
            eos_rate = metrics.get("Stage 6A Natural Holdout", {}).get("eos_rate", 0)
            rep_rate = metrics.get("Stage 6A Natural Holdout", {}).get("rep_rate", 0)
            syn_rate = metrics.get("Stage 6A Natural Holdout", {}).get("syntax_rate", 0)
