import os, sys, json, random, math, torch
...
STAGE6C_TRAIN_FILE = os.path.join(ROOT, "data", "stage6c_hf_full_function_train.jsonl")
...
STEP_CKPT  = os.path.join(CKPT_DIR, "samat_next_350m_stage6c_step_{step}.pt")
FINAL_CKPT = os.path.join(CKPT_DIR, "samat_next_350m_stage6c_best.pt")
...
class MixedStageDataset(Dataset):
    def __init__(self, tok, max_seq):
        ...
        # Load Stage 6C HF Full-Function
        s6 = [json.loads(l) for l in open(STAGE6C_TRAIN_FILE, encoding="utf-8")]
        skipped_6 = 0
        for ex in s6:
        ...
        print(f"Loaded: Stage 6C Full ({len(s6)-skipped_6}), Stage 3 ({len(s3)-skipped_3}), Stage 5 ({len(s5)-skipped_5})")
        
        # Now create the mixed dataset (80/10/10) dynamically when sampling
        self.s6_data = [e for e in self.examples if e.get("source") == "stage6"]
    ...
    def __getitem__(self, idx):
        r = random.random()
        if r < 0.8:
            return random.choice(self.s6_data)
        elif r < 0.9:
...
def main():
    ...
    print(f"Stage 6C Clean Full-Function Coding Generalization | device={device}")
    ...
    while opt_step < TOTAL_STEPS:
        ...
        if opt_step % 50 == 0:
            ...
            eos_rate = metrics.get("Stage 6C HF Full-Function Holdout", {}).get("eos_rate", 1.0)
            rep_rate = metrics.get("Stage 6C HF Full-Function Holdout", {}).get("rep_rate", 0.0)
            syn_rate = metrics.get("Stage 6C HF Full-Function Holdout", {}).get("syntax_rate", 1.0)
            timeout_rate = metrics.get("Stage 6C HF Full-Function Holdout", {}).get("timeout_rate", 0.0)
