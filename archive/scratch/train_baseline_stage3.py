...
Data: data/stage3_paraphrase_train.jsonl
Teacher: N/A
Hyperparams: lr=5e-6, warmup=100, grad_clip=0.5, 2000 optimizer updates, fp32
"""
...
TRAIN_FILE = os.path.join(ROOT, "data", "stage3_paraphrase_train.jsonl")
CKPT_DIR   = os.path.join(ROOT, "checkpoints")
RESULTS    = os.path.join(ROOT, "results", "baseline_stage3_log.json")
os.makedirs(os.path.join(ROOT, "results"), exist_ok=True)

BEST_CKPT  = os.path.join(CKPT_DIR, "transformer_350m_baseline_stage3_best.pt")
STEP_CKPT  = os.path.join(CKPT_DIR, "transformer_350m_baseline_stage3_step_{step}.pt")
BASE_CKPT  = os.path.join(CKPT_DIR, "transformer_350m_baseline_stage2a_best.pt")
...
def main():
    ...
    dataset = Stage3Dataset(TRAIN_FILE, tok, MAX_SEQ)
    loader  = DataLoader(dataset, batch_size=BATCH, shuffle=True,
                         collate_fn=collate, drop_last=True)
                         
    model.load_state_dict(torch.load(BASE_CKPT, map_location=device, weights_only=True))
    print(f"Loaded baseline Stage 2A checkpoint: {BASE_CKPT}")
