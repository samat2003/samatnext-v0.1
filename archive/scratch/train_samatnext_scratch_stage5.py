"""
Stage 5 Training: SamatNext-350M Scratch Test (Experiment A)
======================================================
Start from: Scratch (Random Init)
Data: data/stage5_teacher_distill.jsonl
Teacher: Qwen2.5-Coder-3B-Instruct
Hyperparams: lr=1e-4, warmup=300, grad_clip=0.5, 2000 optimizer updates, fp32
"""
...
RESULTS    = os.path.join(ROOT, "results", "samatnext_scratch_stage5_log.json")
os.makedirs(os.path.join(ROOT, "results"), exist_ok=True)

BEST_CKPT  = os.path.join(CKPT_DIR, "samatnext_350m_scratch_stage5_best.pt")
STEP_CKPT  = os.path.join(CKPT_DIR, "samatnext_350m_scratch_stage5_step_{step}.pt")
...
def main():
    ...
    print(f"SamatNext Scratch Stage 5 Training | device={device}")
    ...
    # We DO NOT load the checkpoint! Random init
    ...
    print(f"\n{'='*70}")
    print(f"Starting SamatNext training from scratch: {TOTAL_STEPS} optimizer updates")
    print(f"{'='*70}\n")
    ...
    while opt_step < TOTAL_STEPS:
        ...
        if accum_loss < best_loss:
            ...
            # don't save best ckpt during smoke test
            if TOTAL_STEPS > 100:
        ...
        if opt_step % SAVE_EVERY == 0 and TOTAL_STEPS > 100:
    ...
    print(f"\n{'='*70}")
    print(f"Stage 5 SamatNext Scratch Training Complete")
    print(f"{'='*70}")
