...
def main():
    ...
    parser.add_argument("--steps",  type=int, default=500,  help="Number of optimizer steps to run")
    ...
    log_data      = []
    best_loss     = float("inf")
    best_ckpt_path = os.path.join(ROOT, "checkpoints", "samat_next_350m_stage2a_fixed_best.pt")
    nan_happened  = False
    ...
    for batch_idx in range(args.steps * GRAD_ACCUM):
        ...
        if (batch_idx + 1) % GRAD_ACCUM == 0:
            ...
            if step_count % SAVE_EVERY == 0:
                ckpt = os.path.join(
                    ROOT, "checkpoints",
                    f"samat_next_350m_stage2a_fixed_step_{step_count}.pt"
                )
    ...
    # Final save
    final_ckpt = os.path.join(ROOT, "checkpoints", "samat_next_350m_stage2a_fixed_latest.pt")
