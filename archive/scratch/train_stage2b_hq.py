...
def main():
    ...
    # Load dataset
    data_path = os.path.join(ROOT, "data", "stage2b_hq_data.jsonl")
    data = []
    ...
    log_data      = []
    best_loss     = float("inf")
    best_ckpt_path = os.path.join(ROOT, "checkpoints", "samat_next_350m_stage2b_hq_best.pt")
    nan_happened  = False
    ...
    for batch_idx in range(args.steps * GRAD_ACCUM):
        ...
        if (batch_idx + 1) % GRAD_ACCUM == 0:
            ...
            if step_count % SAVE_EVERY == 0:
                ckpt = os.path.join(
                    ROOT, "checkpoints",
                    f"samat_next_350m_stage2b_hq_step_{step_count}.pt"
                )
    ...
    # Final save
    final_ckpt = os.path.join(ROOT, "checkpoints", "samat_next_350m_stage2b_hq_latest.pt")
    torch.save(model.state_dict(), final_ckpt)

    # Persist log
    log_file = os.path.join(ROOT, "results", "stage2b_hq_log.json")
