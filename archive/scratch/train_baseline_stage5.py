...
Hyperparams: lr=3e-4, warmup=300, grad_clip=0.5, 2000 optimizer updates, fp32
"""
...
RESULTS    = os.path.join(ROOT, "results", "baseline_stage5_log.json")
os.makedirs(os.path.join(ROOT, "results"), exist_ok=True)

BEST_CKPT  = os.path.join(CKPT_DIR, "transformer_350m_baseline_stage5_best.pt")
STEP_CKPT  = os.path.join(CKPT_DIR, "transformer_350m_baseline_stage5_step_{step}.pt")
...
SAVE_EVERY  = 500
SAMPLE_EVERY = 100
BASE_LR     = 1e-4      # User requested 1e-4
WARMUP      = 300       # Warmup is longer than total steps for smoke test, so it will just linearly increase
GRAD_CLIP   = 0.5
SEED        = 42
